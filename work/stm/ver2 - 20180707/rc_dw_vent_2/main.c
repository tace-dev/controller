...
extern void	vent_loop(uint8_t key);
extern void	vent_loopCb(uint8_t key);
...
volatile uint8_t	tct[KEY_SIZE];
volatile uint8_t	_tctimer[KEY_SIZE];
volatile uint8_t	_tctimer100ms[KEY_SIZE];
...
void	deviceCb(void)
{
	...
	while( _uart1_rx_tail != _uart1_rx_head )
	{
		...
		if( i == 0xff )
		{
			...
			_tcKey = _touchKey(0, 0x01, 0x01);
			_tcKey |= _touchKey(1, 0x02, 0x02);
			_tcKey |= _touchKey(2, 0x04, 0x04);
			_tcKey |= _touchKey(3, 0x08, 0x08);
			_tcKey |= _touchKey(4, 0x10, 0x10);
			_tcKey |= _touchKey(5, 0x20, 0x20);
			_tcKey |= _touchKey(6, 0x40, 0x40);
			_tcKey |= _touchKey(7, 0x80, 0x80);
#endif
			if( _tcKey == 0 )
			{
				_tcKeyPrev = 0xff;
			...
			if( _tcKeyPrev != _tcKey )
			{
				...
				_key_continue = 0;
					...
					if( _lcdTimer[1] == 2 )
					{
						...
						_backup_backlight2 = BACKLIGHT_MAX_LEVEL;
#if defined (_RC_DW_) || defined (_RC_DW_VENT_)
						_touchOn();
#endif
						//	skip this key event
...
//						_tcKeyPrev = 0xff;
			...
			{
#if defined (_RC_DW_VENT_)
				if (_tcKeyPrev == 0x04 || _tcKeyPrev == 0x40) {
					...
					else
					{
						if( _tctimer100ms[_i0Prev] > TIME_LONG_TOUCH )
						{
							...
							if( _tcKeyPrev == 0x08 )
							{
								_key_continue = 1;
								_tctimer100ms[_i0Prev] = 0;
								vent_loop(_tcKeyPrev | 0xf0);
							}
						}
						if( _tctimer100ms[_i0Prev] > TIME_LONG_TOUCH2 )
						{
							if( _tcKeyPrev == 0x10 )
							{
								_key_continue = 1;
								_tctimer100ms[_i0Prev] = 0;
								disp_status(_tcKeyPrev + 0x0f);
							}
							if( _tcKeyPrev == 0x80 )
							{
								_key_continue = 1;
...
*/	
	...
	GPIO_Init(D_485v_PORT, D_485v_PIN, GPIO_Mode_Out_PP_Low_Fast);
	GPIO_Init(GPIOA, GPIO_Pin_2, GPIO_Mode_Out_PP_Low_Fast);
...
Entry:		
		...
		*/
		...
		if( logo < 99 )
		{
			if( __timer100ms != pp[1] )
			{
				pp[1] = __timer100ms;
				logo = logo==25 ? 100 : logo+1;
			}
			continue;
		}		
