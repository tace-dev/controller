...
void	disp_status_loop_message(uint8_t i)
{
	...
	{	
		_disp_mode = 0x01;	
		LCD_BIT_SET(bT9);	//	현재온도
		LCD_BIT_SET(bT11);	//	도
		LCD_BIT_SET(bT10);	//	설정온도
		LCD_BIT_SET(bT12);	//	도
		LCD_BIT_SET(bT16);	//	난방	
	}
	else if( i == 1 )	/*	(1)예약*/	
...
//	else if( _ds == 67 )	disp_status_temp_offset(key, 68);	//	if opMode==0, skip
	...
	{
		//_dsCount = 0;
		disp_status_toffset(key, 34);	//	mainR/C only
	}
	else if( _ds == 34 )	
	{
