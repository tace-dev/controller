...
void cnt_ptc_save_flash(uint8_t b0, uint8_t page) {
	FUNCINFO *p;
	p = (((b0 & 0x80) >> 7) == 0) ? &funcInfo : &funcInfo2;
	switch (page) {
		case 0:
			if (((b0 & 0x80) >> 7) == 0) {
			...
			p->boiler_type = fi[0].boiler_type;
			p->plc = fi[0].plc;
			p->lpmType = fi[0].lpmType;
			p->relay = fi[0].relay;
			p->cnt = fi[0].cnt;
			p->valve = fi[0].valve;
			p->boiler_set_water_shower = fi[0].boiler_set_water_shower;
			p->houseCapa = fi[0].houseCapa;
			
			p->boiler_set_water_heat = fi[0].boiler_set_water_heat;
			
			p->subrcLock = fi[0].subrcLock;
			p->poType = fi[0].poType;
			p->cntlMode = fi[0].cntlMode;
			p->opMode = fi[0].opMode;
			break;
		case 1:
			if (((b0 & 0x80) >> 7) == 0) {
			...
			p->valve_rc[0] = fi[1].valve_rc[0];
			p->valve_rc[1] = fi[1].valve_rc[1];
			p->valve_rc[2] = fi[1].valve_rc[2];
			p->valve_rc[3] = fi[1].valve_rc[3];
			p->valve_len[0] = fi[1].valve_len[0];
			p->valve_len[1] = fi[1].valve_len[1];
			p->valve_len[2] = fi[1].valve_len[2];
			p->valve_len[3] = fi[1].valve_len[3];
			break;
		case 2:
			if (((b0 & 0x80) >> 7) == 0) {
			...
			p->valve_rc[4] = fi[2].valve_rc[4];
			p->valve_rc[5] = fi[2].valve_rc[5];
			p->valve_rc[6] = fi[2].valve_rc[6];
			p->valve_rc[7] = fi[2].valve_rc[7];
			p->valve_len[4] = fi[2].valve_len[4];
			p->valve_len[5] = fi[2].valve_len[5];
			p->valve_len[6] = fi[2].valve_len[6];
			p->valve_len[7] = fi[2].valve_len[7];
			break;
	}
	/*	
	*/
...
//		p = (((mode & 0x80) >> 7) == 0) ? &funcInfo : &funcInfo2;
	...
	{
		_okjung_page = 0;
		cnt_ptc_save_flash(mode, 0 );
		cnt_ptc_save_flash(mode, 1 );
		cnt_ptc_save_flash(mode, 2 );
	}
	return;
#endif
