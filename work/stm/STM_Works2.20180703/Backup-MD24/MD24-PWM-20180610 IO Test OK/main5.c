...
void OnCaptureTim2(void)
{
	...
	{
		Pwm.T2 = TIM2_GetCapture2();	
		TIM2_InterruptClear(TIM2_IT_CC2)
//		Pwm.Count = Pwm.T2 > Pwm.T1 ? Pwm.T2 - Pwm.T1 : 0xffff - Pwm.T1 + Pwm.T2;
		Pwm.Count = Pwm.T2 - Pwm.T1;
		Pwm.Calc = 1;
	}
#else
	u16 t;
...
//		dutyInfo.i = (dutyInfo.i + 1) % MAX_DUTYBUFFER;
//		dutyInfo.b[dutyInfo.i] = (t>>4);
	...
	while (1)
	{
		...
		else
		{
			if ( Diff(Timer.Motor, 100) == 1 )
			{
				...
				{
					//dutyInfo.i = (dutyInfo.i + 1) % MAX_DUTYBUFFER;
					//Drive.Step = stepCalc(stepPrepare(dutyInfo.i));
				}
			}
		}
		if( Pwm.Calc == 1 )
		{
			...
			_t1 = Pwm.T1;
			_t2 = Pwm.T2;
			dt = 10000. * _t1 / (_t1 + _t2);
			Pwm.Duty = (uint16_t)dt;
	//		Pwm.Duty = _t1 > _t2 ? 100 * _t1 / _t2 : 100 * _t2 / _t1;
	//		Pwm.Duty = (uint32_t)_t1 * MAX_MOTOR_POS / _t2;
