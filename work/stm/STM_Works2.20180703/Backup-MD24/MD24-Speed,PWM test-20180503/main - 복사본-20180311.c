...
void StartDrive( uint16_t Target );
...
void main(void)
{
  ...
  //EXTI_SetExtIntSensitivity(EXTI_PORT_GPIOB, EXTI_SENSITIVITY_RISE_ONLY);
	...
	Drive.ExcitationType = 2;		//	2 excition
	...
	while (1)
  {
		...
		//if ( Ticks_1mS > ( Timer.Motor + 500) )
		if ( Diff ( Timer.Motor, 500u ) )
		{
			//GPIO_WriteReverse(_LED_PORT, _LED_PIN);
			Drive.Step = (u16)Adc.Average * 100 /105; 
			...
			if ( Drive.Step != Drive.PreStep && My.Mode == 0  )
			{		
		...
		if ( Diff ( Timer.CycleRun, 60000u ) )
		{	
				if ( Drive.Position == 0 )
					StartDriveOR( CPulse( 200u), 0u );
				else
					StartDriveOR( CPulse( 0u), 0u );			
					
			Timer.CycleRun = Ticks_1mS;
		}
		
		if ( Timer.MotorUpdate )
		{
			if ( Diff( Timer.MotorDelay, 200 ) )
			{			
				StartDriveOR( CPulse(Drive.Step), 0u );	//Drive.Step, 0u  );
				Timer.MotorUpdate = 0;
			}
          ...
          if ( Drive.Position == 0 && ! Drive.Run )
				...
				case 2 : // Power On ;
					if ( Adc.Value[3] < 80 )
					{
						...
						Drive.Interval = My.OffInterval;
						
						Drive.ExcitationType = 0;		//12 excition
						
						StartDriveOR( 0u, 0u );
						Drive.Stage = 3u ;		
						
					}
					else if ( Adc.Value[3] > 800 )
					{
						Drive.pos = Adc.Value[0]/8u;
						Drive.pos *= 8u;
					  if ( Drive.pos > 950u) Drive.pos = 950;
						
						if ( (PrePos/8) != (Drive.pos/8) )
						{
							PrePos = Drive.pos ;
						  ...
						  GPIO_WriteLow( _BOOST_ENABLE_PORT, _BOOST_ENABLE_PIN ); 
							...
							StartDriveOR(  PrePos * Drive.Ratio, 0u );	
						}
					}
					break;
