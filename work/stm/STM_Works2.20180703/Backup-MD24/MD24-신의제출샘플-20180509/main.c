...
void main(void)
{
  ...
  //EXTI_SetExtIntSensitivity(EXTI_PORT_GPIOB, EXTI_SENSITIVITY_RISE_ONLY);
	...
	Drive.Position = 0;
	Drive.Interval = 12u;	//	for 24V 35RD Motor Best speed
	Drive.Interval = 11u;	//	for 24V 35RD Motor Best speed
	Drive.Interval = 14u;	//	for 24V 35RD Motor Best speed
...
TIM2_PRESCALER_1  = ((uint8_t)0x00),
  ...
  TIM2_PRESCALER_32768 = ((uint8_t)0x0F)
	*/
	Drive.Prescaler = TIM2_PRESCALER_8;
		...
		else if ( Drive.Stage == 1 )	// Running 
		{
			...
			//if ( (tt-Timer.Motor) > 1000 )
			{
				...
				else
				{
					...
					sDuty -= Duty[iDuty];
					Duty[iDuty] = Drive.PwmDuty;
					sDuty += Duty[iDuty]; //Drive.Step; 
					//Drive.Step = (sDuty /8+1)/2;
					//mDuty = ((sDuty / nDuty)*2 +1)/2 ;
...
void OnCaptureTim2(void)
{
	...
	{		
		...
		if ( Drive.PwmT2 > Drive.PwmT1 ) 
		{
			if ( Drive.PwmOF == 0 )
			{
				Drive.PwmT4 = Drive.PwmT2 - Drive.PwmT1;
			}
			else
			{
				Drive.PwmT4 = 0xffff;
		...
		else		
    {
			if ( Drive.PwmOF == 1 )
			{
				Drive.PwmT4 = 0xffff - Drive.PwmT1 + Drive.PwmT2;
			}
			else
			{
				Drive.PwmT4 = 0xffff;
		...
		if ( Drive.PwmT3 == 0xffff || Drive.PwmT4 == 0xffff )
		{
			Drive.PwmDuty = 0u;
		}
		else
		{
			Drive.PwmDuty = ( Drive.PwmT3 * (uint32_t)400 / ( Drive.PwmT3 + Drive.PwmT4 ) ) + 1 ;
			Drive.PwmDuty /= 2;
		}
	...
	else
	{
		...
		if ( Drive.PwmT1 > Drive.PwmT2 ) 
		{
			if ( Drive.PwmOF == 0 )
			{
				Drive.PwmT3 = Drive.PwmT1 - Drive.PwmT2;
			}
			else
			{
				Drive.PwmT3 = 0xffff;
		...
		else		
    {
			if ( Drive.PwmOF == 1 )
			{
				Drive.PwmT3 = 0xffff - Drive.PwmT2 + Drive.PwmT1;
			}
			else
			{
				Drive.PwmT3 = 0xffff;
