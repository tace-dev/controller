...
void	_disp_status_loop_host_boiler(uint8_t id)
{
	...
	*/
	...
	{
		//	ON
		__boiler_shower[6] = _boiler_water_shower;
	}
	else
	{
		//	OFF
...
//				ENC_MOVE_0(40,80,5);
			...
			else if( key == 3 )	{	EXIT_HOST(0);	__s = 4;	}
	...
	else if( __s == 1 )
	{
		...
		else if( _dsCount == 1 )
		{
			...
			if( key == 1 )		{	_B_CNTL(1) EXIT_HOST(0);	}
			else if( key == 2 )	{	_B_CNTL(1) EXIT_HOST3(0);	}
			else if( key == 3 )	{	
				__s = 2;
				_dsCount = 0;
				lcd_blink_clear();
