  ...
  59                     ; 153 	return (uint8_t)(_uart1_rx_head != _uart1_rx_tail ? 1 : 0); 
  ...
  68  000b 4f            	clr	a
  71  000c 81            	ret	
 104                     ; 156 void	uart1_tx_485(uint8_t s)
 104                     ; 157 {
 105                     .text:	section	.text,new
 106  0000               _uart1_tx_485:
 110                     ; 166 	COM1_ITConfig(COM1, COM1_IT_TXE, ENABLE);
 ...
 119                     ; 170 }
 122  000e 81            	ret	
 181                     ; 188 void	uart1_tx_put(uint8_t *buf, uint8_t size)
 181                     ; 189 {
 ...
 193                     ; 193 	for( i=0; i<size; i++ )
 195  0004 0f02          	clr	(OFST+0,sp)
 197  0006 2029          	jra	L56
 198  0008               L16:
 199                     ; 195 		TxBuffer1[_uart1_tx_head] = buf[i];
 ...
 213  0019 f6            	ld	a,(x)
 214  001a 85            	popw	x
 215  001b d70014        	ld	(_TxBuffer1,x),a
 216                     ; 196 		_uart1_tx_head = (uint8_t)((_uart1_tx_head + 1) % TX1_BUFFER_SIZE);
 ...
 302  0011 85            	popw	x
 303                     ; 209 	if( _uart1_tx_head == _uart1_tx_tail )
 305  0012 2610          	jrne	L501
 306                     ; 212 		COM1_ITConfig(COM1, COM1_IT_TXE, DISABLE);
 ...
 319  0023 81            	ret	
 320  0024               L501:
 321                     ; 218 	i = TxBuffer1[_uart1_tx_tail];
 323  0024 c60002        	ld	a,__uart1_tx_tail
 324  0027 5f            	clrw	x
 325  0028 97            	ld	xl,a
 326  0029 d60014        	ld	a,(_TxBuffer1,x)
 327  002c 6b01          	ld	(OFST+0,sp),a
 328                     ; 219 	_uart1_tx_tail = (uint8_t)((_uart1_tx_tail + 1) % TX1_BUFFER_SIZE);
 ...
 465  001e 81            	ret	
 499                     ; 236 uint8_t	uart1_pkt_ready(void)
 499                     ; 237 {
 500                     .text:	section	.text,new
 501  0000               _uart1_pkt_ready:
 503  0000 88            	push	a
 504       00000001      OFST:	set	1
 507                     ; 238 	uint8_t	i = (uint8_t)((_uart1_rx_head - 1 + RX1_BUFFER_SIZE) % RX1_BUFFER_SIZE);
 509  0001 c60001        	ld	a,__uart1_rx_head
 510  0004 5f            	clrw	x
 511  0005 97            	ld	xl,a
 512  0006 1c000f        	addw	x,#15
 513  0009 a610          	ld	a,#16
 514  000b cd0000        	call	c_smodx
 516  000e 9f            	ld	a,xl
 517  000f 6b01          	ld	(OFST+0,sp),a
 518                     ; 239 	return (uint8_t)(RxBuffer1[i]==/*PKT_485_ETX*/0 ? 1 : 0);
 520  0011 5f            	clrw	x
 521  0012 97            	ld	xl,a
 ...
 532  0020 81            	ret	
 564                     ; 242 void	systemUart(void)
 564                     ; 243 {
 565                     .text:	section	.text,new
 566  0000               _systemUart:
 570                     ; 247 	CLK_PeripheralClockConfig(CLK_Peripheral_USART1, ENABLE);
 572  0000 ae0501        	ldw	x,#1281
 573  0003 cd0000        	call	_CLK_PeripheralClockConfig
 575                     ; 250 	GPIO_ExternalPullUpConfig(D_USART_TX_PORT, D_USART_TX_PIN, ENABLE);
 577  0006 4b01          	push	#1
 578  0008 4b08          	push	#8
 579  000a ae500a        	ldw	x,#20490
 580  000d cd0000        	call	_GPIO_ExternalPullUpConfig
 582  0010 85            	popw	x
 583                     ; 254 	GPIO_ExternalPullUpConfig(D_USART_RX_PORT, D_USART_RX_PIN, ENABLE);
 585  0011 4b01          	push	#1
 586  0013 4b04          	push	#4
 587  0015 ae500a        	ldw	x,#20490
 588  0018 cd0000        	call	_GPIO_ExternalPullUpConfig
 590  001b 85            	popw	x
 591                     ; 256 	USART_Init(COM1, 
 591                     ; 257 		9600, 
 591                     ; 258 		USART_WordLength_8b, 
 591                     ; 259 		USART_StopBits_1,
 591                     ; 260 		USART_Parity_No, 
 591                     ; 261 		(USART_Mode_TypeDef)(USART_Mode_Tx | USART_Mode_Rx
 591                     ; 262 		)
 591                     ; 263 	);
 ...
 605                     ; 264 	COM1_ITConfig(COM1, COM1_IT_RXNE, ENABLE);
 ...
 614                     ; 265 	COM1_ITConfig(COM1, COM1_IT_TC, ENABLE);
 ...
 623                     ; 266 	COM1_Cmd(USART1, ENABLE);
 ...
 633  0066 84            	pop	a
 634                     ; 290 	_uart1_rx_head = 0;
 636                     ; 291 	_uart1_rx_tail = 0;
 638                     ; 292 	_uart1_tx_head = 0;
 640                     ; 293 	_uart1_tx_tail = 0;
 642                     ; 325 }
 645  0067 81            	ret	
 669                     ; 327 void	systemUart3ChangeEvenParity(void)
 669                     ; 328 {
 670                     .text:	section	.text,new
 671  0000               _systemUart3ChangeEvenParity:
 675                     ; 354 }
 678  0000 81            	ret	
 702                     ; 356 void	systemUart3ChangeNoneParity(void)
 702                     ; 357 {
 ...
 796  0004               _RxBuffer1:
 797  0004 000000000000  	ds.b	16
 798                     	xdef	_RxBuffer1
 799  0014               _TxBuffer1:
 800  0014 000000000000  	ds.b	8
 801                     	xdef	_TxBuffer1
