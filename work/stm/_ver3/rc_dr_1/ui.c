...
//
uint8_t	_backup_backlight = 0x19;
...
void	disp_status_loop_message(uint8_t i)
{
	...
	{	
		_disp_mode = 0x01;	
		/*
			Not Display 'ON' by Heedop 2015.10.25
		LCD_BIT_SET(bP12);	
		*/
		LCD_BIT_SET(bP19);	
		LCD_BIT_SET(bP20);	
...
//		LCD_BIT_SET(bP15);	
	...
	else if( i == 5 )	{	_disp_mode = 0x04;	LCD_BIT_SET(bP11);	/*	(2)정지*/	}
...
void	disp_status(uint8_t key)
{
	...
	if( _key_continue == 1 )
	{
//		if( key==2 )		{		goto SetupMaster;	}
		if( key==4 )	{	disp_statusCb(200);}
			//goto SetupSub;	}
//		else if( key==8 )	{ copy_info(150);}		
		//goto SetupRoomCntl; }
//		else if( key==1 )	{		goto SetupRc; }//goto SetupCommand;	}
//		else if( key==0x10 ){}		//goto SetupRc;	//}
		//	long key
		else if( key==0x55 )
		{
			//	ID
