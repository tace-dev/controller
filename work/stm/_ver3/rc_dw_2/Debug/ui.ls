  ...
  54  0010 80            	dc.b	128
 100                     ; 73 void lcd_disp_rid_hw(uint8_t id, uint8_t t) 
 100                     ; 74 {
 102                     .text:	section	.text,new
 103  0000               _lcd_disp_rid_hw:
 107                     ; 76 	_led_key_status = t==0 ? 1 : 0;
 109  0000 9f            	ld	a,xl
 110  0001 4d            	tnz	a
 ...
 123                     ; 79 		D_IO_HIGH(GPIOD, GPIO_Pin_7);
 125  000f 721e500f      	bset	20495,#7
 128  0013 81            	ret	
 129  0014               L72:
 130                     ; 81 		D_IO_LOW(GPIOD, GPIO_Pin_7);
 132  0014 721f500f      	bres	20495,#7
 133                     ; 83 }
 136  0018 81            	ret	
 179                     ; 85 void	error_display(uint8_t type, uint8_t err)
 179                     ; 86 {
 180                     .text:	section	.text,new
 181  0000               _error_display:
 183  0000 89            	pushw	x
 184       00000000      OFST:	set	0
 187                     ; 87 	if( error_display_mode == 1 )
 ...
 534  013c               L531:
 535                     ; 200 	} else if (_disp_mode2 == RC_MODE_REPEAT) {
 ...
 544  0146               L141:
 545                     ; 202 	} else if (_disp_mode2 == RC_MODE_STOP) {
 ...
 554  0150               L541:
 555                     ; 204 	} else if (_disp_mode2 == RC_MODE_OUT) {
 ...
 818                     ; 248 	_boiler_water_shower = (_boiler_water_shower/5)*5;
 820  029b 5f            	clrw	x
 821  029c 97            	ld	xl,a
...
1006                     ; 331 	FW_CNT_MODE(uiInfo.controlMode);
...
1014                     ; 332 	FW_VALVE(uiInfo.valve);
...
1022                     ; 333 	FW_VALVE_SUB(uiInfo.valve_sub);
...
1030                     ; 334 	FW_REPEAT(uiInfo.repeat);
...
1038                     ; 335 	FW_RESERVE(uiInfo.reserve);
...
1046                     ; 336 	FW_TSETUP(uiInfo.tSetup);
...
1054                     ; 338 	FW_POTYPE(uiInfo.poType);
...
1062                     ; 339 	FW_HOUSECAPA(uiInfo.houseCapa);
...
1070                     ; 340 	FW_TOFFSET(uiInfo.tOffset);
...
1334                     ; 345 	FW_B_SHOWER(_boiler_water_shower);
...
1342                     ; 346 	FW_B_HEAT(_boiler_water_heat);
...
1350                     ; 347 	FW_TEMPOFFSET(uiInfo.tempOffset);
...
1358                     ; 348 	FW_LPM(uiInfo.lpmType);
...
1366                     ; 349 	FW_LPM_SUB(uiInfo.lpmType_sub);
...
1374                     ; 350 	FW_DISPMODE(_disp_mode);
...
1601                     ; 395 void	disp_status_id(uint8_t key, uint8_t next)
1601                     ; 396 {
...
1729  007c               L313:
1730                     ; 440 	else if( key == 0x20 )	__DN
...
1745  0088               L713:
1746                     ; 441 	else if( key == 4 )	
1748  0088 a104          	cp	a,#4
1749  008a 2623          	jrne	L513
1750                     ; 443 		uiInfoSet.mode = uiInfoSet.id == 1 ? 1 : 0;
...
1771                     ; 445 		_backup_backlight = 0;
1774  00a6 725f0009      	clr	__backup_backlight
1775                     ; 447 		_backlight = __bL;
1777  00aa 5500180000    	mov	__backlight,___bL
1778  00af               L513:
1779                     ; 449 }
1782  00af 5b03          	addw	sp,#3
1783  00b1 81            	ret	
1838                     ; 451 void	disp_status_valveCount(uint8_t key, uint8_t next)
1838                     ; 452 {
...
1872                     ; 463 		if( uiInfo.enc != 0 )
1874  001e c60001        	ld	a,_uiInfo
1875  0021 2731          	jreq	L353
1876                     ; 466 			if( (uiInfo.enc & 0x40) != 0 )
...
1974  0085               L773:
1975                     ; 490 	else if( key == 0x20 )	__DN
...
2025  00b6 81            	ret	
2090                     ; 502 void	disp_status_toffset(uint8_t key, uint8_t next)
2090                     ; 503 {
...
2204  0075               L564:
2205                     ; 543 			uiInfoSet.tOffset = i;
2207  0075 c70079        	ld	_uiInfoSet+11,a
2208  0078               L744:
2209                     ; 545 		if( _dsPrev != i )
...
2218                     ; 548 			clear_npos2();
2220  0084 cd0000        	call	_clear_npos2
2222                     ; 549 			if( i < 100 && i != 0 )
...
2269  00bd               L505:
2270                     ; 558 	else if( key == 0x20 )	__UP
...
2704  0084               L536:
2705                     ; 657 	else if( key == 0x20 )	__UP
...
2720  0090               L146:
2721                     ; 658 	else if( key == 4 )	
2723  0090 a104          	cp	a,#4
2724  0092 2617          	jrne	L736
2725                     ; 663 		FW_BACKLIGHT(_backlight);
...
2749  00ad 81            	ret	
2805                     ; 668 void	disp_status_run_op(uint8_t key, uint8_t next)
2805                     ; 669 {
...
2862                     ; 688 			uiInfoSet.opMode = i;
2864  002c c70072        	ld	_uiInfoSet+4,a
2865  002f               L776:
2866                     ; 690 		if( _dsPrev != i )
...
2875                     ; 693 			clear_npos2();
2877  003b cd0000        	call	_clear_npos2
2879                     ; 694 			if( i==0 )		lcd_disp_setup_cntl_on();
...
2911  0059               L117:
2912                     ; 699 	else if( key == 0x20 )	__UP
...
2927  0065               L517:
2928                     ; 700 	else if( key == 4 )		
2930  0065 a104          	cp	a,#4
2931  0067 260d          	jrne	L317
2932                     ; 702 		STATUS_NEXT(next);	
...
2951  0078 81            	ret	
3008                     ; 706 void	disp_status_cntl_op(uint8_t key, uint8_t next)
3008                     ; 707 {
...
3065                     ; 726 			uiInfoSet.cntlMode = i;
3067  002c c7006e        	ld	_uiInfoSet,a
3068  002f               L357:
3069                     ; 728 		if( _dsPrev != i )
...
3078                     ; 731 			clear_npos2();
3080  003b cd0000        	call	_clear_npos2
3082                     ; 732 			if( i == 0 )	lcd_disp_setup_mode_local();
...
3092  0047               L757:
3093                     ; 733 			else			lcd_disp_setup_mode_each();
3095  0047 cd0000        	call	_lcd_disp_setup_mode_each
3097  004a               L157:
3098                     ; 736 	if( key == 0x10 )		__DN
...
3110  0056               L367:
3111                     ; 737 	else if( key == 0x20 )	__UP
...
3126  0062               L767:
3127                     ; 738 	else if( key == 4 )		
3129  0062 a104          	cp	a,#4
3130  0064 260d          	jrne	L567
3131                     ; 740 		STATUS_NEXT(next);	
...
3150  0075 81            	ret	
3205                     ; 744 void	disp_status_houseCapa(uint8_t key, uint8_t next)
3205                     ; 745 {
...
3354  0089               L1401:
3355                     ; 779 	else if( key == 0x20 )	__UP
...
3370  0095               L5401:
3371                     ; 780 	else if( key == 4 )		
3373  0095 a104          	cp	a,#4
3374  0097 260d          	jrne	L3401
3375                     ; 782 		STATUS_NEXT(next);	
...
3394  00a8 81            	ret	
3427                     ; 786 void	disp_status_po_type(uint8_t key, uint8_t next)
3427                     ; 787 {
...
3503                     ; 839 void	disp_status_lpm(uint8_t key, uint8_t next)
3503                     ; 840 {
...
3618                     ; 880 			clear_npos2();
3620  006a cd0000        	call	_clear_npos2
3622                     ; 881 			UI_DISP_2Digit(__lpm[i]);
...
3653  008e               L3411:
3654                     ; 885 	else if( key == 0x20 )	__UP
...
3669  009b               L7411:
3670                     ; 886 	else if( key == 4 )		
3672  009b a104          	cp	a,#4
3673  009d 260d          	jrne	L5411
3674                     ; 888 		STATUS_NEXT(next);	
...
3702  00b9 81            	ret	
3735                     ; 892 void	disp_status_temp_offset(uint8_t key, uint8_t next)
3735                     ; 893 {
...
3748  0019               __ui_:
3749  0019 00            	dc.b	0
3829                     ; 940 void	disp_status_valveinfo_sub(uint8_t key, uint8_t next, uint8_t prev)
3829                     ; 941 {
...
4048  00e5               L1621:
4049                     ; 991 	else if( key == 0x20 )	__UP
...
4249  01aa 81            	ret	
4332                     ; 1034 void	disp_status_valveinfo(uint8_t key, uint8_t next, uint8_t prev)
4332                     ; 1035 {
...
4341                     ; 1038 	if( uiInfoSet.controlMode==1 )
4343  0003 c60074        	ld	a,_uiInfoSet+6
4344  0006 4a            	dec	a
4345  0007 260d          	jrne	L7631
4346                     ; 1040 		disp_status_valveinfo_sub(key, next, prev);
...
4569  00f9               L7241:
4570                     ; 1090 	else if( key == 0x20 )	__UP
...
4777  01cc 81            	ret	
4831                     ; 1127 void	disp_status_plc(uint8_t key, uint8_t next)
4831                     ; 1128 {
...
5069                     ; 1217 		copy_infoSet();
5071  0015 cd0000        	call	_copy_infoSet
5073                     ; 1218 		FW_MODE(uiInfo.mode);
...
5089                     ; 1220 		FW_TOFFSET(uiInfo.tOffset);
...
5117                     ; 1226 			FW_VALVE(uiInfo.valve);
...
5125                     ; 1227 			FW_VALVE_SUB(uiInfo.valve_sub);
...
5141                     ; 1229 			FW_CNT_SUB(uiInfo.cnt_sub);
...
5149                     ; 1230 			FW_CNT_MODE(uiInfo.controlMode);
...
5157                     ; 1231 			FW_OPMODE(uiInfo.opMode);
...
5165                     ; 1232 			FW_POTYPE(uiInfo.poType);
...
5173                     ; 1233 			FW_HOUSECAPA(uiInfo.houseCapa);
...
5181                     ; 1234 			FW_LPM(uiInfo.lpmType);
...
5189                     ; 1235 			FW_TEMPOFFSET(uiInfo.tempOffset);
...
5591                     ; 1272 		copy_infoSet();
5593  0015 cd0000        	call	_copy_infoSet
5595                     ; 1273 		FW_MODE(uiInfo.mode);
...
5611                     ; 1275 		FW_TOFFSET(uiInfo.tOffset);
...
5635                     ; 1282 			FW_VALVE(uiInfo.valve);
...
5643                     ; 1283 			FW_VALVE_SUB(uiInfo.valve_sub);
...
5659                     ; 1285 			FW_CNT_SUB(uiInfo.cnt_sub);
...
5667                     ; 1286 			FW_CNT_MODE(uiInfo.controlMode);
...
5675                     ; 1287 			FW_OPMODE(uiInfo.opMode);
...
5683                     ; 1288 			FW_POTYPE(uiInfo.poType);
...
5691                     ; 1289 			FW_HOUSECAPA(uiInfo.houseCapa);
...
5699                     ; 1290 			FW_LPM(uiInfo.lpmType);
...
5707                     ; 1291 			FW_TEMPOFFSET(uiInfo.tempOffset);
...
6108                     ; 1327 	lcd_clear(0);
6110  0029 cd0000        	call	_lcd_clear
6112                     ; 1328 	for( i=0; i<4; i++ )
6114  002c 0f01          	clr	(OFST+0,sp)
6115  002e               L5761:
6116                     ; 1329 		lcd_disp_n(i, _dsCount%10);
...
6205  0090               L5071:
6206                     ; 1361 		__func_rc_timerout1 = 0;
...
6338                     ; 1385 		LCD_BIT_SET(bT6);	//	난방	
6340  0030 c60006        	ld	a,_iLF_DEF+6
6341  0033 cd00c0        	call	LC035
6342  0036 ca0006        	or	a,_bLF_DEF+6
6344  0039 2078          	jp	LC032
6345  003b               L5471:
6346                     ; 1387 	else if( i == 1 )	/*	(1)예약*/	
6348  003b a101          	cp	a,#1
6349  003d 2605          	jrne	L1571
6350                     ; 1389 		_disp_mode = 0x08;	
6352  003f cd00d8        	call	LC037
6353                     ; 1392 		LCD_BIT_SET(bT14);	//	예약
6354  0042 206f          	jp	LC032
6355  0044               L1571:
6356                     ; 1394 	else if( i == 2 )	/*	(2)반복*/	
6358  0044 a102          	cp	a,#2
6359                     ; 1396 		_disp_mode = 0x10;	
6360                     ; 1399 		LCD_BIT_SET(bT18);	//	타이머	
6362  0046 275c          	jreq	LC033
6363                     ; 1410 	else if( i == 8 )		/*	(1)외출*/	
6365  0048 a108          	cp	a,#8
6366  004a 2618          	jrne	L1671
6367                     ; 1412 		_disp_mode = 0x02;	
6369                     ; 1413 		LCD_BIT_SET(bT1);	//	현재온도
6371                     ; 1414 		LCD_BIT_SET(bT3);	//	도
6373                     ; 1415 		LCD_BIT_SET(bT7);	//	외출
6375  004c               LC034:
...
6472  00b6               L7471:
6473                     ; 1466 	_disp_mode2 = _disp_mode;
6475  00b6 5500000000    	mov	__disp_mode2,__disp_mode
6476                     ; 1467 	timerClear();
...
6509  00e7 81            	ret	
6556                     ; 1473 void	disp_status_loop_init(void)
6556                     ; 1474 {
...
6648                     ; 1541 		uiInfo._temp[3] = uiInfo.tSetup;
6651  0056 55000f0064    	mov	_uiInfo+99,_uiInfo+14
6652                     ; 1542 		uiInfo.tSetup = 20;
6654  005b 2006          	jp	LC038
6655  005d               L7502:
6656                     ; 1550 	}else if( __sUp == 17 )	//	Out long
6658  005d a111          	cp	a,#17
6659  005f 2606          	jrne	L3602
6660                     ; 1552 		__ui_disp_temp_c(1);
6662  0061 ad0c          	call	LC039
6663                     ; 1555 		uiInfo.tSetup = 20;
6665  0063               LC038:
6667  0063 3514000f      	mov	_uiInfo+14,#20
6669  0067               L3602:
6670                     ; 1557 	else if( __sUp == 151 )	//	OUT
6673                     ; 1563 	disp_status_loop_message(__sUp);
...
6841  0039               L7312:
6842                     ; 1617 		error_display_mode = 0;
6844  0039 725f000c      	clr	_error_display_mode
6845  003d               L5312:
6846                     ; 1620 	if( ui_error == 0 && ui_error_type == 0 )
6848  003d c60000        	ld	a,_ui_error
6849  0040 2611          	jrne	L1412
6851  0042 725d0000      	tnz	_ui_error_type
6852  0046 260b          	jrne	L1412
6853                     ; 1622 		goto RESET_ENTRY;
6854  0048               L5012:
6855                     ; 1609 RESET_ENTRY:			
6855                     ; 1610 		/*
6855                     ; 1611 			reset mode
6855                     ; 1612 		 */
6855                     ; 1613 		if (ui_error == 30 && ui_error_type == ERROR_TYPE_CNT) {
...
6869  0053 85            	popw	x
6870  0054 81            	ret	
6931                     ; 1628 void	disp_status_command(uint8_t key)
6931                     ; 1629 {
...
7012                     ; 1651 			uiInfo._t = i;
7014  0065 c70060        	ld	_uiInfo+95,a
7015  0068               L1122:
7016                     ; 1653 		if( _dsPrev != i )
...
7025                     ; 1656 			clear_npos2();
7027  0074 cd0000        	call	_clear_npos2
7029                     ; 1657 			if( i==0 )		lcd_disp_setup_cntl_tab();
...
7150  00de               L7622:
7151                     ; 1670 	else if( key == 0x20 )	__DN
...
7166  00eb               L3722:
7167                     ; 1671 	else if( key == 4 )		
7169  00eb a104          	cp	a,#4
7170  00ed 2661          	jrne	L7722
7171                     ; 1673 		switch( uiInfo._t )
...
7194  010f               L5412:
7195                     ; 1675 		case 0:
7195                     ; 1676 			uiInfoSet.opMode = 3;	
7197  010f 35030072      	mov	_uiInfoSet+4,#3
7198                     ; 1677 			disp_status_run(3, 35, 0);
7200                     ; 1678 			break;
7202  0113 202a          	jp	LC043
7203  0115               L7412:
7204                     ; 1679 		case 1:	
7204                     ; 1680 			uiInfoSet.opMode = 2;	
7206  0115 35020072      	mov	_uiInfoSet+4,#2
7207                     ; 1681 			disp_status_run(3, 35, 0);
7209                     ; 1682 			break;
7211  0119 2024          	jp	LC043
7212  011b               L1512:
7213                     ; 1683 		case 2:	
7213                     ; 1684 			uiInfoSet.opMode = 0;	
7215  011b c70072        	ld	_uiInfoSet+4,a
7216                     ; 1685 			disp_status_run(3, 35, 0);
7218                     ; 1686 			break;
7220  011e 201f          	jp	LC043
7221  0120               L3512:
7222                     ; 1687 		case 3:	
7222                     ; 1688 			uiInfoSet.opMode = 1;	
7224  0120 35010072      	mov	_uiInfoSet+4,#1
7225                     ; 1689 			disp_status_run(3, 35, 0);
7227                     ; 1690 			break;
7229  0124 2019          	jp	LC043
7230  0126               L5512:
7231                     ; 1692 			STATUS_NEXT(95);
7233  0126 355f0014      	mov	__ds,#95
7236                     ; 1693 			break;
7239  012a 202b          	jp	LC041
7240  012c               L7512:
7241                     ; 1694 		case 6:	
7241                     ; 1695 			func_rc_command(COMMAND_MODE_FACTORY, 100);	
...
7250                     ; 1697 		case 7:	
7250                     ; 1698 			//	MAIN
7250                     ; 1699 			func_rc_command(COMMAND_MODE_FACTORY, 110);	
7252  0134 ae086e        	ldw	x,#2158
7254                     ; 1700 			disp_status_run(3, 35, 0);
7256                     ; 1701 			break;
7258  0137 2003          	jp	LC044
7259  0139               L3612:
7260                     ; 1702 		case 8:	
7260                     ; 1703 			//	SUB
7260                     ; 1704 			func_rc_command(COMMAND_MODE_FACTORY, 120);	
7262  0139 ae0878        	ldw	x,#2168
7263  013c               LC044:
7264  013c cd0000        	call	_func_rc_command
7266                     ; 1705 			disp_status_run(3, 35, 0);
7269  013f               LC043:
...
7511                     ; 1755 	lcd_disp_n(2, (i & 0x0F)/10);
...
7523                     ; 1756 	lcd_disp_n(3, (i & 0x0F)%10);
...
7714  0021 00            	dc.b	0
7796                     ; 1800 void	disp_statusCb(uint8_t key)
7796                     ; 1801 {
...
7874  0055               L3442:
7875                     ; 1832 	else if( _ds == 36 )	disp_status_toffset(key, 37);
...
7888  0065               L7442:
7889                     ; 1833 	else if( _ds == 37 )	
7891  0065 a125          	cp	a,#37
7892  0067 2617          	jrne	L3542
7893                     ; 1835 		t = disp_status_run(key, 38, 1);
...
7910  0080               L3542:
7911                     ; 1839 	else if( _ds == 38 )	disp_status_exit(key, 100);
...
7924  0090               L1642:
7925                     ; 1841 	else if( _ds == 2 )		disp_status_id(key, 3);
...
7938  00a0               L5642:
7939                     ; 1842 	else if( _ds == 3 )	
7941  00a0 a103          	cp	a,#3
7942  00a2 2617          	jrne	L1742
7943                     ; 1844 		t = disp_status_run_id(key, 38, 2);
...
7960  00bb               L1742:
7961                     ; 1849 	else if( _ds == 9 )		disp_status_run_op(key, 61);
...
7974  00cb               L7742:
7975                     ; 1850 	else if( _ds == 61 )	disp_status_cntl_op(key, 63);
...
7988  00db               L3052:
7989                     ; 1851 	else if( _ds == 63 )	disp_status_valveCount(key, 64);
...
8002  00eb               L7052:
8003                     ; 1852 	else if( _ds == 64 )	disp_status_houseCapa(key, /*65*/66);	//	if opMode==0, skip
...
8016  00fb               L3152:
8017                     ; 1855 	else if( _ds == 66 )	disp_status_lpm(key, /*67*/68);		//	세대밸브 if cntlMode==1, skip
...
8030  010b               L7152:
8031                     ; 1859 	else if( _ds == 68 )	disp_status_valveinfo(key, 69, 68);
...
8046  011e               L3252:
8047                     ; 1860 	else if( _ds == 69 )	disp_status_plc(key, 70);
...
8060  012e               L7252:
8061                     ; 1861 	else if( _ds == 70 )	
8063  012e a146          	cp	a,#70
8064  0130 2617          	jrne	L3352
8065                     ; 1863 		t = disp_status_run(key, 71, 9);
...
8082  0149               L3352:
8083                     ; 1867 	else if( _ds == 71 )	disp_status_exit(key, 100);
...
8096  0159               L1452:
8097                     ; 1869 	else if( _ds == 10 )	
8099  0159 a10a          	cp	a,#10
8100  015b 2617          	jrne	L5452
8101                     ; 1871 		t = disp_status_run(key, 11, 9);
...
8118  0174               L5452:
8119                     ; 1875 	else if( _ds == 11 )	disp_status_exit(key, 100);
...
8132  0184               L3552:
8133                     ; 1877 	else if( _ds == 12 )	disp_status_exit(key, 100);
...
8146  0194               L7552:
8147                     ; 1878 	else if( _ds == 15 )	
8149  0194 a10f          	cp	a,#15
8150  0196 2616          	jrne	L3652
8151                     ; 1880 		t = disp_status_run(key, 11, 20);
...
8168  01ae               L3652:
8169                     ; 1884 	else if( _ds == 33 )	disp_status_toffset(key, 34);	//	mainR/C only
...
8182  01bd               L1752:
8183                     ; 1885 	else if( _ds == 34 )	
8185  01bd a122          	cp	a,#34
8186  01bf 2616          	jrne	L5752
8187                     ; 1887 		t = disp_status_run(key, 35, 33);
...
8204  01d7               L5752:
8205                     ; 1891 	else if( _ds == 35 )	disp_status_exit(key, 100);
...
8218  01e6               L3062:
8219                     ; 1893 	else if( _ds == 41 )	disp_status_command(key);
...
8277  0223 81            	ret	
8278  0224               L7162:
8279                     ; 1910 	else if (_ds == 150)							// 온도보상
8281  0224 a196          	cp	a,#150
8282  0226 260b          	jrne	L5262
8283                     ; 1912 		disp_status_toffset(key, 151);	//	mainR/C only
...
8292  0233               L5262:
8293                     ; 1914 	else if (_ds == 151)							// 백라이트
8295  0233 a197          	cp	a,#151
8296  0235 260b          	jrne	L1362
8297                     ; 1916 		disp_status_backlight2(key, 153);
...
8306  0242               L1362:
8307                     ; 1918 	else if (_ds == 152) 
8309  0242 a198          	cp	a,#152
8310  0244 2616          	jrne	L5362
8311                     ; 1920 		t = disp_status_run(key, 153, 150);
...
8357  0279               L7462:
8358                     ; 1934 	else if (_ds == 161)							// 백라이트
8360  0279 a1a1          	cp	a,#161
8361  027b 260b          	jrne	L3562
8362                     ; 1936 		disp_status_backlight2(key, 162);
...
8405  02ae               L3662:
8406                     ; 1950 	else if( _ds == 81 )	{	disp_write_file(12);	}
...
8417  02ba               L1762:
8418                     ; 1951 	else if( _ds == 95 )	{	disp_485(12);	}
...
8429  02c6               L5762:
8430                     ; 1952 	else if( _ds == 96 )	{	disp_write_file(12);	}
...
8587  037a               L3472:
8588                     ; 2019 	else if( _ds == 221 )
8590  037a a1dd          	cp	a,#221
8591  037c 26f9          	jrne	L5442
8592                     ; 2021 		FW_ID(uiInfoSet.id);
...
8600                     ; 2022 		FW_MODE(uiInfoSet.mode);
...
9003                     ; 2121 		_updateTimer = __timer1s;
9005  001a 5500000005    	mov	__updateTimer,___timer1s
9006                     ; 2122 		_updateCount--;
9008  001f 725a0007      	dec	__updateCount
9009                     ; 2123 		if( _updateCount == 0 )
9011  0023 265f          	jrne	L5403
9012                     ; 2125 			if( (_updateBit&BIT_REPEAT) != 0 )	{FW_REPEAT(uiInfo.repeat);	}
...
9023  0034               L1503:
9024                     ; 2126 			if( (_updateBit&BIT_RESERVE) != 0 )	{FW_RESERVE(uiInfo.reserve);}
...
9064  0064               L5503:
9065                     ; 2138 			if( (_updateBit&BIT_DISPMODE) != 0 ){
9067  0064 720700060a    	btjf	__updateBit,#3,L3603
9068                     ; 2139 				if (uiInfo.mode == SETTING_MODE_SUB) {
9070                     ; 2141 						FW_DISPMODE(_disp_mode);
9073                     ; 2144 					FW_DISPMODE(_disp_mode);
...
9125  009c               L1013:
9126                     ; 2170 		if( key==8 )	{ copy_info(150);}		
