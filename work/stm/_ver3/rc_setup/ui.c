...
//
CONST	uint8_t	__lpm[] = { 0, 8, 10, 15, 20, 25, 30, 99 };
...
extern void	_ui_setup(uint8_t key);
extern uint8_t _ui_writer(uint8_t key);
...
void	disp_statusCb(uint8_t key)
{
	...
	else if( _ds == 64 )	disp_status_houseCapa(key, 65);	//	if opMode==0, skip
	else if( _ds == 65 )	disp_status_po_type(key, 66);	//	비례제어
	else if( _ds == 66 )	disp_status_lpm(key, 67);		//	세대밸브 if cntlMode==1, skip
	else if( _ds == 67 )	disp_status_temp_offset(key, 68);	//	if opMode==0, skip
																//	clear valve info
	else if( _ds == 68 )	disp_status_valveinfo(key, 69, 68);
	else if( _ds == 69 )	disp_status_plc(key, 69);	
	else if( _ds == 70 )	
	{
...
//#define _CNTL_SETUP_		
	...
	{
		uiInfoSet.id = 1;
		uiInfoSet.lpmType = 1;		//	8LPM
		uiInfoSet.mode = 1;			//	1 host
		uiInfoSet.controlMode = 0;	//	0 main, 1 sub
		uiInfoSet.valve_sub = 0;
		uiInfoSet.poType = 0;
		uiInfoSet.tOffset = 100;
		uiInfoSet.tempOffset = 100;
		uiInfoSet.lpmType_sub = 0;
		uiInfoSet.subRCLock = 0;
		//__cntlMode = uiInfoSet.cntlMode = 1;	//	0 local, 1 each
		_boiler_type = 0;		//	0 normal, 1 plc
		uiInfoSet.plc = 0;		
		
		disp_status_run_op(key, 221);	//	PO, On/Off
	}
	else if( _ds == 221 )	disp_status_cntl_op(key, 223);	//	Local, Each
	else if( _ds == 223 )	disp_status_valveCount(key, 224);
	else if( _ds == 224 )	disp_status_houseCapa(key, 225);	//	if opMode==0, skip
	else if( _ds == 225 )	disp_status_valveinfo(key, 226, 225);
	else if( _ds == 226 )	
	{
		//t = disp_status_run(key, 227, 220);
		//if( t == 1 )
		//	_ds = 220;
		copy_infoSet();
		_ds = 228;
	}
	else if( _ds == 227 )	disp_status_exit(key, 220);
	else if( _ds == 228 )
	{
		if( _ui_writer(key) == 1 )
			_ds = 220;
...
//		}
	...
	else if( _ds == 99 )
	{
		...
		{
			/*
				설정프로그램에서는 에러 처리 화면을 표시하지 않도록 수정
				2015.10.1
			 */
			/*
			ui_error = error;
