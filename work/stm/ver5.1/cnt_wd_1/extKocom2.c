...
void	km2CheckRange(uint8_t *b)
{
	LPKM2INFO	p = get_km2Info();
	if( b[11] == 0x01 )	
		b[12] = p->dn;
		
	if( b[12] > p->up )		b[12] = p->up;
	else if( b[12] < p->dn )	b[12] = p->dn;
...
#define KOCOM_IF_CURR_ROOM_OVER_EXIT	{\
					if( funcInfo.cnt+funcInfo2.cnt < kocom_curr_room+1 )\
					{\
						_k2[6] = _k2[0] = 0;\
						__kretry[0] = 0;\
						return;\
...
uint8_t hn_kocom2_ptc_base_check(void) {
	...
	if( _ua.b[6] != 0xff )
	{
		if( funcInfo.cnt+funcInfo2.cnt < (_ua.b[6]+1) )
			return 0;
	}
	*/
	return 1;
...
void hn_kocom2_make_set_packet(uint8_t count, uint8_t rid) 
{
	uint8_t	r;
	for( r=0; r<21; r++ )
		_ua.r[r] = _k2buf[r];
...
//	_ua.r[12] = (rcInfo.setup_temp[rid] / 2);
	...
	_ua.r[15] = 0x00; // 추후 지원 (난방수 현재 온도)
	_ua.r[16] = funcInfo.error;
	_ua.r[17] = /*(hn_kocom2_check_room_state(rid) == 1)*/_uc.r[14]>_uc.r[12] ? 0x02 : 0x01;
	hn_kocom2_make_checkSum();
	KOCOM_SEND
}
...
//	_uc.r[10] |= (_is_shower==1) ? 0x04 : 0x01;
	_uc.r[12] = r!=MODE_HOT ? rcInfo._t[rid] : _uc.r[62]/2;
	...
	_uc.r[17] = /*(hn_kocom2_check_room_state(rid) == 1)*/_uc.r[14]>_uc.r[12] ? 0x02 : 0x01;
...
void	hn_kocom_cb(uint8_t mode)
{
	...
	{
		//	0x9c + 0x3a + n
		if( _ua.b[6]+1 > funcInfo.cnt+funcInfo2.cnt )
		{
			_k2[0] = 0;
			return;
		}
		hn_kocom2_make_response_packet(0, _ua.b[6]);
		_k2[0] = 11;
		KOCOM_RETRY
	}
	else if( _k2[0] == 11 )
	{
		if( _k2[1] == 1 )		{	_k2[0] = _k2[1] = 0;	}
		if( __kretry[4] == 1 )
		{
			__kretry[4] = 0;
	...
	else if( _k2[0] == 21 )
	{
		...
		{
			KOCOM_RETRY
			hn_kocom2_make_set_packet(0, kocom_curr_room);
			_k2[0] = 25;
			kocom_write = kocom_curr_room+1;
		}
	}
	else if( _k2[0] == 25 )
	{
		...
		{
			__kretry[4] = 0;
			hn_kocom2_make_set_packet(kocom_retry_count, kocom_curr_room);			
		}
	}
	else if( _k2[0] == 22 )
	{
	...
	else if( _k2[0] == 5 )
	{
		...
		hn_kocom2_make_response_packet(kocom_retry_count, kocom_curr_room);
		_k2[0] = 55;
		_k2[6] = 1;	//	ff : continue if not ack ready
	}
	else if( _k2[0] == 55 )
	{
		if( _k2[1] == 1 )
...
//	_hn_wait_ready = 1;
	...
	else
	{
		...
		if( _ua.b[9] == 0x00 )
		{
			if( kocom_curr_room != 0xff )
			{
				_k2[0] = funcInfo.cnt+funcInfo2.cnt < kocom_curr_room+1 ? 4 : 2;
				if( _k2[0] == 4 )
					return;
			}
			hnInfo_kocom.on_off = (_ua.b[10] & 0x10) ? 1 : 0;
			t = _ua.b[10] & 0x0F;
		...
		else if( _ua.b[9] == 0x3a )
		{
			if( kocom_curr_room != 0xff )
			{
				_k2[0] = funcInfo.cnt+funcInfo2.cnt < kocom_curr_room+1 ? 4 : 7;
			}
			else
				_k2[0] = 8;
		}
